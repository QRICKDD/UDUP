...
def evaluate_and_draw(model,adv_patch, image_root, gt_root,pan_cfg,
                      save_path,model_name,scale_factor):
    ...
    for img, name, gt in zip(images, image_names, test_gts):
        ...
        merge_image=transforms.Resize([int(h * scale_factor), int(w * scale_factor)])(merge_image)
...
def main(patch_path,img_path,gt_path,save_path,model_name,scale_factor):
    pan_cfg=None
    if model_name=="craft":
        model = load_CRAFTmodel()
    ...
    P, R, F=evaluate_and_draw(model=model,adv_patch=adv_patch,image_root=img_path,
                              ...
                              model_name=model_name,scale_factor=scale_factor)
    del model
    return R, P, F,torch.mean(1-adv_patch)


def gen_iter_dict(special_eval_item):
    mypath=os.path.join(os.path.join('.'),'result_save_0.06',special_eval_item)
...
if __name__=="__main__":
    ...
    root_eval = os.path.join(abspath, 'result_save_0.06')#保存的advpatch路径
    img_path = os.path.join(abspath, "AllData", "test_scale")#resize
    ...
    for dir in special_eval:#****************patch+lambda cycle****************
        #====================================================Filter====================================================
        ...
        #====================================================Log====================================================
        log_path = os.path.join(abspath, 'Mylog_0.06\eval_scale_log')
        log_name = os.path.join(abspath, 'Mylog_0.06\eval_scale_log\{}.log'.format(sele_size))
        #clean logger
        ...
        for scale_factor in resize_scales:#****************scales cycle****************
            for mui,iter in zip(muis,iter_eval):#****************mui cycle****************
                if mui in muis_filter:
                    continue
                logger.info("iter={},mui={}".format(iter,mui))
                patch_path = os.path.join(p1,"advtorch","advpatch_{}".format(iter))

                #for model_name in ["psenet","dbnet","easyocr","panpp","panet","craft"]:#*******model cycle*****
                for model_name in ["craft"]:  # *******model cycle*****
                    gt_path=os.path.join(abspath,"AllData","test_"+model_name+"_scale_gt",str(scale_factor))
                    scales_save_path = os.path.join(abspath,"scale_save", dir,
                                                    model_name,str(iter)+'_'+str(mui),str(scale_factor))
                    if not os.path.exists(scales_save_path):
                        os.makedirs(scales_save_path)
                    R,P,F,pert = main(patch_path, img_path, gt_path, scales_save_path, model_name,scale_factor)
                    logger.info("model:{},iter:{} mui:{} scale_factor:{} R:{:.4f} P:{:.4f} F:{:.4f} MUI:{:.4f}".
                            format(model_name,iter,mui,scale_factor, R, P, F,pert))